...
{
  const num = agent.parameters.p_n;
  //ph=num;
  const client = new MongoClient(url);
  await client.connect();
  const dbobj = await client.db("chatbot").collection("user_table").findOne({phno: num});
  ...
  {
  user_name=dbobj.username;
  await agent.add("Welcome  "+user_name+"!  \n How can I help you?"); console.log("fun 1 complete");}
}
...
function report_issue(agent)
{
  ...
  var dbo = db.db("chatbot");
    ...
    //trouble_ticket+=time_date+""+date_ob.getHours()+""+date_ob.getMinutes()+""+date_ob.getSeconds();
